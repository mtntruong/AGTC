...
class RPCA_Block(nn.Module):
    def __init__(self, tensor_num_channels):
        ...
        self.Proximal_Q = RDB(tensor_num_channels, growRate0=64, growRate=32, nConvLayers=8)
...
class RPCA_Net(nn.Module):
    def __init__(self, N_iter, tensor_num_channels):
        ...
        # Final weighted sum
        self.composer = nn.Conv2d(self.tensor_num_channels, 3, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, img0, img1, img2, omg0, omg1, omg2):
        ...
        # Init L/R
        L = torch.ones((self.tensor_num_channels, 3, OmegaD.shape[-1]), device=torch.device('cuda')) / 1e2
