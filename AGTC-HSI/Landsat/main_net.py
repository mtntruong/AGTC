...
class RDB(nn.Module):
    def __init__(self, growRate0, growRate, nConvLayers):
        ...
        # In/Out conv
        self.in_conv = nn.Conv2d(in_channels=8, out_channels=64, kernel_size=3, stride=1, padding=1)
        self.out_conv = nn.Conv2d(in_channels=64, out_channels=8, kernel_size=3, stride=1, padding=1)
...
class RPCA_Net(nn.Module):
    def __init__(self, N_iter):
        ...
        # Weight for RLS
        self.att_module = SpatialAttentionModule(8)
    ...
    def forward(self, data, omega):
        ...
        # Init L/R
        L = torch.ones((8, 2, OmegaD.shape[-1]), device=torch.device('cuda')) / 1e2
